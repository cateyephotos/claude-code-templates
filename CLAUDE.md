...
8. **Vercel cleanUrls** — two distinct gotchas, both rooted in `cleanUrls: true` + `trailingSlash: false`:
   - **Relative-path resolution on hub pages.** Stripping the trailing slash from `/{dir}/` → `/{dir}` makes the browser resolve relative `href=`/`src=` values against the site root, not the directory — breaking stylesheets, scripts, and links. **Fix:** add `<base href="/{dir}/">` to the `<head>` of any `/{dir}/` hub page. Caught on `/interactions/` (2026-04-18, commit `0124f6c3`).
   - **Rewrite destinations ending in `.html` silently 404.** When a Vercel rewrite points at a destination like `/stacks-view.html`, cleanUrls intercepts the `.html` suffix inside the rewrite chain and produces a 404 instead of serving the file. **Fix:** drop the `.html` from rewrite destinations — use `"destination": "/stacks-view"` (the cleanUrl form), not `"destination": "/stacks-view.html"`. Caught on `/stacks/:slug+ → /stacks-view` dynamic routing (2026-04-19, commit `6d6cd1de` follow-up). The `/guides/:slug → /guides/:slug.html` pattern does not trip this because `:slug` is substituted into the destination — Vercel resolves the result relative to the filesystem, and cleanUrls handles the final serve.
