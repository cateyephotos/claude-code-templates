...
- `.content-gate-icon` reuses the existing 56px circular gradient icon container (`content-gate.css` line 125) with an SVG lock icon — NOT an emoji.
...
**Chosen approach:** Update the idempotency guard to also check for `clerk-key`:
```javascript
if (html.includes('id="auth-buttons"') || html.includes("clerk-publishable-key") || html.includes('name="clerk-key"')) {
```
Then handle supplement pages specially: skip the full `HEAD_INJECT` (already present from `seed.js`), but still inject `content-gate.css` in `<head>` and `monograph-gate.js` before `</body>`. This is lower-risk than modifying `seed.js`'s auth injection and keeps the existing supplement generation pipeline unchanged.
