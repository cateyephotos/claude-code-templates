...
(async () => {
  console.log('🔧 TESTING GINKGO BILOBA FIX');
  console.log('Verifying Ginkgo Biloba data structure fixes...');
  ...
  try {
    ...
    // Test Ginkgo Biloba specifically
    console.log('\n🔍 Testing Ginkgo Biloba (ID: 14)...');
    
    await page.fill('#searchInput', 'Ginkgo Biloba');
    await page.waitForTimeout(1000);
    
    await page.evaluate(async () => {
      await window.app.showSupplementDetails(14);
    });
    ...
    const benefitsResult = await page.evaluate(() => {
      const container = document.getElementById('benefits-14');
    ...
    const safetyResult = await page.evaluate(() => {
      const container = document.getElementById('safety-14');
      if (!container) return { error: 'Container not found' };
    ...
    const mechanismsResult = await page.evaluate(() => {
      const container = document.getElementById('mechanisms-14');
      if (!container) return { error: 'Container not found' };
    ...
    console.log('\n' + '='.repeat(50));
    console.log('🔧 GINKGO BILOBA FIX RESULTS');
    ...
    if (overallWorking) {
      console.log(`\n🎉 GINKGO BILOBA COMPLETELY FIXED!`);
    ...
    await browser.close();
    console.log('\n✅ Ginkgo Biloba fix test complete');
  }
})();
