...
(async () => {
  console.log('🔧 TESTING QUERCETIN FIX');
  console.log('Verifying Quercetin safety tab fix...');
  ...
  try {
    ...
    // Test Quercetin specifically
    console.log('\n🔍 Testing Quercetin (ID: 20)...');
    
    await page.fill('#searchInput', 'Quercetin');
    await page.waitForTimeout(1000);
    
    await page.evaluate(async () => {
      await window.app.showSupplementDetails(20);
    });
    ...
    const benefitsResult = await page.evaluate(() => {
      const container = document.getElementById('benefits-20');
    ...
    // Test Safety tab (the one we fixed)
    ...
    const safetyResult = await page.evaluate(() => {
      const container = document.getElementById('safety-20');
      if (!container) return { error: 'Container not found' };
    ...
    if (safetyResult.firstCardText) {
      console.log(`   First Card: ${safetyResult.firstCardText}...`);
    }
    ...
    const mechanismsResult = await page.evaluate(() => {
      const container = document.getElementById('mechanisms-20');
      if (!container) return { error: 'Container not found' };
    ...
    const safetyWorking = safetyResult.studyCards > 0 && !safetyResult.hasUndefined;
    const mechanismsWorking = mechanismsResult.studyCards > 0 && !mechanismsResult.hasUndefined;
    
    console.log('\n' + '='.repeat(50));
    console.log('🔧 QUERCETIN FIX RESULTS');
    ...
    if (overallWorking) {
      console.log(`\n🎉 QUERCETIN COMPLETELY FIXED!`);
      console.log(`   All tabs now show detailed citations.`);
    } else {
      console.log(`\n⚠️ PARTIAL FIX:`);
