...
class ModernSupplementDatabase {
    ...
    async _attachEnhancedCitations() {
        try {
            const results = await this.enhancedCitationAttacher.attachEnhancedCitations(this.supplements);
            ...
            if (results.enhancedAttached > 0) {
                console.log(`📋 Enhanced supplements:`, results.attachedSupplements.slice(0, 5).map(s => s.name));
                if (results.attachedSupplements.length > 5) {
                    console.log(`   ... and ${results.attachedSupplements.length - 5} more`);
                }
            }
            ...
            console.error('❌ Failed to attach enhanced citations:', error);
        }
    }
