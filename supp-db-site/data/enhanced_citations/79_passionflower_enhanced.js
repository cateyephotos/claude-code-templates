// Enhanced Citations for Passionflower (ID: 79)
// Research focus: Anxiety reduction, sleep support, GABAergic mechanisms, preoperative anxiolysis
// Evidence Profile: Tier 2 - Moderate-to-strong evidence with growing clinical support
// Last rebuilt: 2026-03-07 | All PMIDs PubMed-verified

window.enhancedCitations = window.enhancedCitations || {};

window.passionflowerEnhanced = {
    supplementId:  79,
    supplementName:  "Passionflower",
    lastUpdated:  "2026-03-07",
    version:  "2.0",
    evidenceProfile:  {
        overallQuality:  "Tier 2",
        totalCitations:  15,
        researchQualityScore:  76,
        lastEvidenceUpdate:  "2026-03-07",
        evidenceStrength:  {
            mechanisms:  "Strong",
            clinicalBenefits:  "Moderate",
            safety:  "Good",
            dosage:  "Evidence-based"
        },
        researchMaturity:  "Moderate",
        publicationSpan:  "2004-2025",
        keyFindings:  "Traditional herbal anxiolytic (Passiflora incarnata) with well-characterized GABAergic mechanisms including direct GABA-A receptor modulation by flavonoids and chrysin. Clinical trials demonstrate anxiolytic efficacy comparable to benzodiazepines in preoperative settings (500 mg), modest sleep quality improvements in healthy adults, and systematic review support across 9 clinical trials for neuropsychiatric applications. Generally well tolerated with centuries of safe traditional use."
    },
    citations:  {
        mechanisms:  [
            {
                mechanism:  "Passiflora incarnata extracts elicit GABA currents in hippocampal neurons via direct GABAergic...",
                strength:  "Strong",
                evidence:  [
                    {
                        citationId:  "electrophysiology_2010_mechanisms",
                        title:  "",
                        authors:  [],
                        year:  2010,
                        journal:  "",
                        doi:  "",
                        pmid:  "20382514",
                        studyType:  "In Vitro Electrophysiology Study",
                        evidenceLevel:  "Level 2",
                        findings:  "Electrophysiology study demonstrating that Passiflora incarnata extracts elicit GABA currents in hippocampal neurons. GABA itself is a prominent ingredient in passionflower extracts, and its concentration varies significantly depending on extraction method. Water-based extractions yielded higher GABA content. Provides direct mechanistic evidence for GABAergic anxiolytic activity.",
                        methodology:  "In Vitro Electrophysiology Study",
                        sampleSize:  "Rat hippocampal neurons"
                    }
                ]
            },
            {
                mechanism:  "GABAergic and opioidergic dual-pathway mechanisms underlying anxiolytic and antinociceptive effects",
                strength:  "Strong",
                evidence:  [
                    {
                        citationId:  "pharmacological_2016_mechanisms",
                        title:  "",
                        authors:  [],
                        year:  2016,
                        journal:  "",
                        doi:  "",
                        pmid:  "26912265",
                        studyType:  "Animal Pharmacology Study",
                        evidenceLevel:  "Level 2",
                        findings:  "Pharmacological study in mice demonstrating that Passiflora incarnata exerts anxiolytic and antinociceptive effects through both GABAergic and opioidergic pathways. Effects were blocked by flumazenil (GABA-A antagonist) and naloxone (opioid antagonist), confirming dual-pathway mechanism. Supports traditional use as both anxiolytic and analgesic.",
                        methodology:  "Animal Pharmacology Study",
                        sampleSize:  "ICR mice"
                    }
                ]
            },
            {
                mechanism:  "Comprehensive phytochemical profile including flavonoids, glycosides, and alkaloids with...",
                strength:  "Strong",
                evidence:  [
                    {
                        citationId:  "comprehensive_2004_mechanisms",
                        title:  "",
                        authors:  [],
                        year:  2004,
                        journal:  "",
                        doi:  "",
                        pmid:  "15261959",
                        studyType:  "Comprehensive Phytochemical Review",
                        evidenceLevel:  "Level 2",
                        findings:  "Comprehensive review of Passiflora genus identifying key bioactive compounds: C-glycosyl flavonoids (chrysin, vitexin, isovitexin, orientin), indole alkaloids (harman, harmine, harmaline), and the benzoflavone moiety (BZF). Chrysin demonstrates high affinity for GABA-A benzodiazepine receptors. Multiple species show CNS-depressant, anxiolytic, and sedative properties in validated pharmacological models.",
                        methodology:  "Comprehensive Phytochemical Review",
                        sampleSize:  "Chemical and pharmacological data synthesis"
                    }
                ]
            },
            {
                mechanism:  "GABAergic modulation comparable to pharmaceutical anxiolytics in clinical contexts",
                strength:  "Strong",
                evidence:  [
                    {
                        citationId:  "review_2025_mechanisms",
                        title:  "",
                        authors:  [],
                        year:  2025,
                        journal:  "",
                        doi:  "",
                        pmid:  "41392563",
                        studyType:  "Clinical Applications Review",
                        evidenceLevel:  "Level 2",
                        findings:  "Review of Passiflora incarnata anxiolytic potential focusing on GABAergic modulation mechanisms. Passionflower demonstrates anxiolytic effects comparable to midazolam and oxazepam in clinical settings. Active compounds modulate GABA-A receptor subtypes, enhancing inhibitory neurotransmission without the dependence risk associated with benzodiazepines.",
                        methodology:  "Clinical Applications Review",
                        sampleSize:  "Multiple preclinical and clinical studies"
                    }
                ]
            }
        ],
        benefits:  [
            {
                healthDomain:  "Systematic review of 9 clinical trials confirming anxiolytic and neuropsychiatric benefits",
                strength:  "Strong Evidence",
                evidence:  [
                    {
                        citationId:  "systematic_2020_benefits",
                        title:  "",
                        authors:  [],
                        year:  2020,
                        journal:  "",
                        doi:  "",
                        pmid:  "33352740",
                        studyType:  "Systematic Review",
                        evidenceLevel:  "Level 2",
                        findings:  "Systematic review of Passiflora incarnata in neuropsychiatric disorders covering 9 clinical trials. Evidence supports anxiolytic effects across multiple clinical contexts including generalized anxiety, preoperative anxiety, and dental anxiety. No significant adverse effects reported across trials. Also found preliminary evidence for benefits in insomnia, ADHD, and opioid withdrawal.",
                        methodology:  "Systematic Review",
                        sampleSize:  "9 clinical trials reviewed",
                        duration:  "Various"
                    }
                ]
            },
            {
                healthDomain:  "Preoperative anxiety reduction comparable to pharmaceutical anxiolytics at 500 mg",
                strength:  "Strong Evidence",
                evidence:  [
                    {
                        citationId:  "landmark_2008_benefits",
                        title:  "",
                        authors:  [],
                        year:  2008,
                        journal:  "",
                        doi:  "",
                        pmid:  "18499602",
                        studyType:  "Human RCT (Double-blind, Placebo-controlled)",
                        evidenceLevel:  "Level 2",
                        findings:  "Landmark double-blind RCT of 60 patients receiving 500 mg oral Passiflora incarnata or placebo 90 minutes before inguinal herniorrhaphy. Passionflower significantly reduced anxiety (p<0.001) compared to placebo without causing sedation, changes in psychomotor function, hemodynamics, or level of sedation. Demonstrates rapid anxiolytic onset without cognitive impairment.",
                        methodology:  "Human RCT (Double-blind, Placebo-controlled)",
                        sampleSize:  "60 patients undergoing inguinal herniorrhaphy",
                        duration:  "Single dose 90 minutes before surgery"
                    }
                ]
            },
            {
                healthDomain:  "Anxiolysis before spinal anesthesia without sedation or psychomotor impairment",
                strength:  "Strong Evidence",
                evidence:  [
                    {
                        citationId:  "prospective_2012_benefits",
                        title:  "",
                        authors:  [],
                        year:  2012,
                        journal:  "",
                        doi:  "",
                        pmid:  "22048283",
                        studyType:  "Human RCT (Prospective, Double-blind)",
                        evidenceLevel:  "Level 2",
                        findings:  "Prospective double-blind RCT of 60 patients receiving 700 mg oral Passiflora incarnata or placebo 30 minutes before spinal anesthesia. Passionflower suppressed the increase in anxiety without affecting psychomotor test results, sedation level, or hemodynamics. Numerical Rating Scale anxiety scores were significantly lower in the treatment group.",
                        methodology:  "Human RCT (Prospective, Double-blind)",
                        sampleSize:  "60 patients undergoing spinal anesthesia (ASA I-II)",
                        duration:  "Single dose 30 minutes before procedure"
                    }
                ]
            },
            {
                healthDomain:  "Cochrane review finding anxiolytic effects comparable to benzodiazepines",
                strength:  "Moderate Evidence",
                evidence:  [
                    {
                        citationId:  "cochrane_2007_benefits",
                        title:  "",
                        authors:  [],
                        year:  2007,
                        journal:  "",
                        doi:  "",
                        pmid:  "17253512",
                        studyType:  "Cochrane Systematic Review",
                        evidenceLevel:  "Level 3",
                        findings:  "Cochrane systematic review of Passiflora for anxiety disorder. Only 2 studies met inclusion criteria with 198 total participants. One study found passionflower comparable to oxazepam for generalized anxiety with fewer job-related impairment side effects. Authors concluded too few studies to draw firm conclusions but noted promising results warranting further research.",
                        methodology:  "Cochrane Systematic Review",
                        sampleSize:  "2 RCTs, 198 participants",
                        duration:  "Various"
                    }
                ]
            },
            {
                healthDomain:  "Polysomnography-confirmed sleep improvement in adults with insomnia",
                strength:  "Strong Evidence",
                evidence:  [
                    {
                        citationId:  "unknown_2020_benefits",
                        title:  "",
                        authors:  [],
                        year:  2020,
                        journal:  "",
                        doi:  "",
                        pmid:  "31714321",
                        studyType:  "Human RCT (Double-blind, Placebo-controlled)",
                        evidenceLevel:  "Level 2",
                        findings:  "Double-blind RCT of 110 adults with insomnia receiving Passiflora incarnata extract or placebo for 2 weeks. Polysomnography-confirmed significant increase in total sleep time. Represents the largest and most rigorously designed passionflower sleep trial with objective PSG measurement rather than subjective sleep diaries alone.",
                        methodology:  "Human RCT (Double-blind, Placebo-controlled)",
                        sampleSize:  "110 adults aged 20-55 with insomnia",
                        duration:  "2 weeks"
                    }
                ]
            },
            {
                healthDomain:  "Subjective sleep quality improvement with passionflower tea in healthy adults",
                strength:  "Moderate Evidence",
                evidence:  [
                    {
                        citationId:  "unknown_2011_benefits",
                        title:  "",
                        authors:  [],
                        year:  2011,
                        journal:  "",
                        doi:  "",
                        pmid:  "21294203",
                        studyType:  "Human RCT (Double-blind, Placebo-controlled, Crossover)",
                        evidenceLevel:  "Level 3",
                        findings:  "Double-blind crossover RCT of 41 healthy adults. Passionflower herbal tea consumed for 7 days significantly improved subjective sleep quality ratings compared to placebo tea (p<0.01). PSG subsample (10 participants) showed no significant differences, suggesting low-dose tea benefits are primarily subjective. Supports mild sleep-promoting effects at dietary tea-level doses.",
                        methodology:  "Human RCT (Double-blind, Placebo-controlled, Crossover)",
                        sampleSize:  "41 healthy adults aged 18-35",
                        duration:  "1 week per treatment with 1-week washout"
                    }
                ]
            },
            {
                healthDomain:  "Significant stress reduction and sleep improvement with standardized extract",
                strength:  "Moderate Evidence",
                evidence:  [
                    {
                        citationId:  "unknown_2024_benefits",
                        title:  "",
                        authors:  [],
                        year:  2024,
                        journal:  "",
                        doi:  "",
                        pmid:  "38646244",
                        studyType:  "Human RCT (Double-blind, Placebo-controlled)",
                        evidenceLevel:  "Level 3",
                        findings:  "Double-blind RCT of 65 participants receiving standardized Passiflora incarnata extract for 4 weeks. Significant improvement in stress measures and sleep quality scores compared to placebo. Supports the dual anxiolytic-sleep benefit profile of passionflower supplementation in adults with stress-related sleep disturbances.",
                        methodology:  "Human RCT (Double-blind, Placebo-controlled)",
                        sampleSize:  "65 participants with stress and sleep complaints",
                        duration:  "4 weeks"
                    }
                ]
            }
        ],
        safety:  [
            {
                safetyAspect:  "Comprehensive safety profile from ethnopharmacological evidence synthesis",
                strength:  "Good Safety Profile",
                evidence:  [
                    {
                        citationId:  "comprehensive_2013_safety",
                        title:  "",
                        authors:  [],
                        year:  2013,
                        journal:  "",
                        doi:  "",
                        pmid:  "24140586",
                        studyType:  "Comprehensive Ethnopharmacology Review",
                        evidenceLevel:  "Level 3",
                        findings:  "Comprehensive ethnopharmacology review of Passiflora incarnata covering pharmacokinetics, efficacy, and safety. Generally good safety profile established through centuries of traditional use and modern clinical trials. Rare adverse events include dizziness, drowsiness, and confusion at high doses. Caution advised with sedative medications due to potential additive CNS depression. Isolated case reports of hepatotoxicity with multi-herb preparations (causality uncertain).",
                        methodology:  "Comprehensive Ethnopharmacology Review",
                        sampleSize:  "Preclinical, clinical, and pharmacovigilance data"
                    }
                ]
            },
            {
                safetyAspect:  "Strong evidence for anxiolytic efficacy with low adverse event profile across herbal supplements",
                strength:  "Good Safety Profile",
                evidence:  [
                    {
                        citationId:  "systematic_2010_safety",
                        title:  "",
                        authors:  [],
                        year:  2010,
                        journal:  "",
                        doi:  "",
                        pmid:  "20929532",
                        studyType:  "Systematic Review",
                        evidenceLevel:  "Level 3",
                        findings:  "Systematic review of nutritional and herbal supplements for anxiety disorders. Strong evidence found for passionflower with characteristically low adverse event rates compared to conventional anxiolytics. One RCT demonstrated fewer job-related impairment side effects than oxazepam. Supports favorable risk-benefit ratio for anxiety management.",
                        methodology:  "Systematic Review",
                        sampleSize:  "Multiple RCTs of herbal supplements for anxiety"
                    }
                ]
            },
            {
                safetyAspect:  "No significant adverse effects across 9 clinical trials in systematic review",
                strength:  "Good Safety Profile",
                evidence:  [
                    {
                        citationId:  "systematic_2020_safety",
                        title:  "",
                        authors:  [],
                        year:  2020,
                        journal:  "",
                        doi:  "",
                        pmid:  "33352740",
                        studyType:  "Systematic Review",
                        evidenceLevel:  "Level 3",
                        findings:  "Systematic review covering 9 clinical trials of Passiflora incarnata found no significant adverse effects reported in any included study. Occasional mild side effects (drowsiness, dizziness, GI discomfort) reported at low frequency. May potentiate effects of sedative, anxiolytic, and anticoagulant medications. Not recommended during pregnancy due to uterine stimulant alkaloids.",
                        methodology:  "Systematic Review",
                        sampleSize:  "9 clinical trials"
                    }
                ]
            }
        ],
        dosage:  [
            {
                dosageRange:  "Effective acute anxiolytic dose: 500-700 mg extract before procedures",
                strength:  "Evidence-based",
                evidence:  [
                    {
                        citationId:  "preoperative_2008_dosage",
                        title:  "",
                        authors:  [],
                        year:  2008,
                        journal:  "",
                        doi:  "",
                        pmid:  "18499602",
                        studyType:  "Multiple RCTs",
                        evidenceLevel:  "Level 3",
                        findings:  "Preoperative RCTs establish 500-700 mg as effective single-dose range for acute anxiolysis. Movafegh et al. used 500 mg oral dose 90 minutes before surgery with significant anxiety reduction. Aslanargun et al. used 700 mg 30 minutes before spinal anesthesia. Effects onset within 30-90 minutes. Higher end of range may be needed for more acute anxiety situations.",
                        methodology:  "Multiple RCTs",
                        sampleSize:  "120+ surgical patients across studies"
                    }
                ]
            },
            {
                dosageRange:  "Chronic supplementation: 400-800 mg/day extract or 2g dried herb as tea for anxiety and sleep",
                strength:  "Evidence-based",
                evidence:  [
                    {
                        citationId:  "clinical_2020_dosage",
                        title:  "",
                        authors:  [],
                        year:  2020,
                        journal:  "",
                        doi:  "",
                        pmid:  "33352740",
                        studyType:  "Systematic Review & Multiple RCTs",
                        evidenceLevel:  "Level 3",
                        findings:  "Clinical trial dosing ranges from systematic review: standardized extract 400-800 mg/day in divided doses for chronic anxiety and sleep support. Passionflower herbal tea (approximately 2g dried herb per cup) also effective for mild sleep quality improvement. For sleep, consumption 30-60 minutes before bedtime is recommended. Effects may build over 1-4 weeks of consistent use.",
                        methodology:  "Systematic Review & Multiple RCTs",
                        sampleSize:  "Multiple studies synthesized"
                    }
                ]
            }
        ]
    }
};

window.enhancedCitations[79] = window.passionflowerEnhanced;

if (typeof module !== 'undefined' && module.exports) {
    module.exports = window.passionflowerEnhanced;
}
