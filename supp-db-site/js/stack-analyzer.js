...
(function () {
  ...
  function renderSingleGoalResults(content, goal, shared, result) {
    ...
    initExportButtons(shared, result);
  }
  ...
  function renderDualGoalResults(content, analysis, result) {
    ...
    initTabSwitching();
    initExportButtons(analysis, result);
  }
  ...
  function initExportButtons(analysis, result) {
    const copyBtn = $("#sa-copy-btn");
    const copyDropdown = $("#sa-copy-dropdown");

    if (copyBtn && copyDropdown) {
      copyBtn.addEventListener("click", (e) => {
        e.stopPropagation();
        copyDropdown.style.display = copyDropdown.style.display === "none" ? "block" : "none";
      });

      document.addEventListener("click", () => { copyDropdown.style.display = "none"; }, { once: true });

      copyDropdown.querySelectorAll(".sa-copy-option").forEach(option => {
        option.addEventListener("click", async (e) => {
          e.stopPropagation();
          const format = option.dataset.format;
          const text = format === "markdown"
            ? formatResultsAsMarkdown(analysis)
            : formatResultsAsText(analysis);

          try {
            await navigator.clipboard.writeText(text);
            showToast("Copied to clipboard");
            trackEvent("stack_analyzer_copy_results", { format, goal_count: analysis.goals?.length || 1 });
          } catch (err) {
  ...
  function formatResultsAsMarkdown(a) {
    const normalized = a.goals ? a : normalizeAnalysisResult(a, []);
    let md = "# Stack Analysis Results\n\n";

    for (const goal of normalized.goals) {
      md += "## " + goal.goalName + "\n\n";
      md += "**Score:** " + goal.overallScore + "/100 | **Evidence:** " + capitalize(goal.evidenceStrength) + "\n\n";
      md += goal.stackSummary + "\n\n";

      if (goal.mechanismCoverage?.length) {
        md += "### Mechanism Coverage\n\n";
        md += "| Mechanism | Covered By | Strength |\n|-----------|-----------|----------|\n";
        for (const m of goal.mechanismCoverage) {
          md += "| " + m.mechanism + " | " + m.coveredBy.join(", ") + " | " + m.strength + " |\n";
        }
        md += "\n";
      }
    }

    if (normalized.multiGoalCompatibility) {
      const c = normalized.multiGoalCompatibility;
      md += "## Multi-Goal Compatibility\n\n" + c.compatibilityNote + "\n\n";
      if (c.sharedMechanisms?.length) md += "**Shared:** " + c.sharedMechanisms.join(", ") + "\n\n";
      if (c.conflicts?.length) md += "**Conflicts:** " + c.conflicts.join(", ") + "\n\n";
    }

    if (normalized.synergyAnalysis?.length) {
      md += "## Synergy Analysis\n\n";
      for (const s of normalized.synergyAnalysis) {
        md += "- **" + s.pair.join(" + ") + "** (" + s.type + "): " + s.explanation + "\n";
      }
      md += "\n";
    }

    if (normalized.dosageProtocol?.length) {
      md += "## Dosage Protocol\n\n";
      md += "| Supplement | Dose | Timing | Notes |\n|-----------|------|--------|-------|\n";
      for (const d of normalized.dosageProtocol) {
        md += "| " + d.supplement + " | " + d.recommendedDose + " | " + d.timing + " | " + d.notes + " |\n";
      }
      md += "\n";
    }

    md += "\n---\n*Generated by SupplementDB Stack Analyzer*\n";
    return md;
  }

  function formatResultsAsText(a) {
    const normalized = a.goals ? a : normalizeAnalysisResult(a, []);
    let text = "STACK ANALYSIS RESULTS\n\n";

    for (const goal of normalized.goals) {
      text += goal.goalName + ": " + goal.overallScore + "/100 (" + goal.evidenceStrength + " evidence)\n";
      text += goal.stackSummary + "\n\n";
    }

    if (normalized.synergyAnalysis?.length) {
      const synergistic = normalized.synergyAnalysis.filter(s => s.type === "synergistic");
      if (synergistic.length) {
        text += "Key synergies: " + synergistic.map(s => s.pair.join(" + ")).join("; ") + "\n\n";
      }
    }

    if (normalized.safetyFlags?.length) {
      text += "Safety flags: " + normalized.safetyFlags.map(f => f.severity + ": " + f.description).join("; ") + "\n\n";
    }

    text += "Generated by SupplementDB Stack Analyzer\n";
    return text;
  }
  ...
  function showToast(message) {
    ...
    requestAnimationFrame(() => toast.classList.add("sa-toast--visible"));
    setTimeout(() => {
  ...
  function openEmailModal(analysis, result) {
    const auth = window.SupplementDBAuth;
    const userEmail = auth?.user?.primaryEmailAddress?.emailAddress || "";
    const goalNames = (analysis.goals || []).map(g => g.goalName).join(" + ");

    const overlay = document.createElement("div");
    overlay.className = "sa-modal-overlay sa-email-modal-overlay";
    overlay.id = "sa-email-modal";
    overlay.innerHTML = `
      <div class="sa-modal sa-email-modal" role="dialog" aria-labelledby="sa-email-title">
        <div class="sa-modal-header">
          <h3 id="sa-email-title"><i class="fas fa-envelope"></i> Email Results</h3>
          <button class="sa-modal-close" id="sa-email-close"><i class="fas fa-times"></i></button>
        </div>
        <div class="sa-email-form">
          <label class="sa-field-label" for="sa-email-to">Recipient Email</label>
          <input type="email" class="sa-email-input" id="sa-email-to" value="${escapeHtml(userEmail)}" placeholder="your@email.com" />
          <label class="sa-field-label" for="sa-email-subject">Subject</label>
          <input type="text" class="sa-email-input" id="sa-email-subject" value="Your Stack Analysis — ${escapeHtml(goalNames)}" />
          <button class="sa-btn-primary sa-email-send" id="sa-email-send" ${!userEmail ? 'disabled' : ''}>
            <span class="sa-email-send-text"><i class="fas fa-paper-plane"></i> Send</span>
            <span class="sa-email-send-loading" style="display:none"><span class="sa-spinner-sm"></span> Sending...</span>
          </button>
        </div>
      </div>
    `;
    ...
    document.body.appendChild(overlay);
    const emailInput = overlay.querySelector("#sa-email-to");
    emailInput?.focus();

    const sendBtn = overlay.querySelector("#sa-email-send");
    ...
    overlay.querySelector("#sa-email-close")?.addEventListener("click", closeModal);
    overlay.addEventListener("click", (e) => { if (e.target === overlay) closeModal(); });
    overlay.addEventListener("keydown", (e) => { if (e.key === "Escape") closeModal(); });

    sendBtn?.addEventListener("click", async () => {
      const to = overlay.querySelector("#sa-email-to")?.value;
      const subject = overlay.querySelector("#sa-email-subject")?.value;
      if (!to) return;

      const sendText = sendBtn.querySelector(".sa-email-send-text");
      const sendLoading = sendBtn.querySelector(".sa-email-send-loading");
      sendBtn.disabled = true;
      if (sendText) sendText.style.display = "none";
      if (sendLoading) sendLoading.style.display = "inline-flex";

      try {
        await window.SupplementDB.action("stackAnalyzer:emailResults", {
          recipientEmail: to,
          subject: subject,
          analysisMarkdown: formatResultsAsMarkdown(analysis),
        });
        closeModal();
        showToast("Results emailed successfully");
        trackEvent("stack_analyzer_email_results", { goal_count: analysis.goals?.length || 1, success: true });
      } catch (err) {
        const msg = err?.data || err?.message || "Email could not be sent. Please try again.";
        if (msg.includes("rate limit") || msg.includes("limit")) {
          showToast("Email limit reached (5/hour). Try again later.");
        } else {
          showToast(msg);
        }
        sendBtn.disabled = false;
        if (sendText) sendText.style.display = "inline-flex";
        if (sendLoading) sendLoading.style.display = "none";
        trackEvent("stack_analyzer_email_results", { goal_count: analysis.goals?.length || 1, success: false });
      }
    });
  }
